...
description: Fetch Confluence Data Center pages as local PDFs, HTML copies, and page attachments with the confluence-downloader CLI before reviewing them with an agent. Use when the user asks to review, summarize, audit, compare, analyze, or work from Confluence pages, page trees, spaces, attachments, or bulk page configs, especially when pages may not already be downloaded locally and should be pulled only if missing or changed.
---
...
1. Identify the request shape:
   ...
   - Files attached to pages are needed, or the review must account for embedded Google Drive files: add `--download-attachments`.
...
Run commands from the `confluence-downloader` repository root:

```bash
uv run confluence-downloader bulk --config pages.json --output-dir ./pdfs --verbosity normal
```

For one or more named pages:

```bash
uv run confluence-downloader download --space DOC --title "Architecture Overview" --output-dir ./pdfs
```
...
```bash
uv run confluence-downloader download --space DOC --title "Architecture Overview" --include-children --output-dir ./pdfs
```

With page attachments and a standalone HTML copy per page:

```bash
uv run confluence-downloader download --space DOC --title "Architecture Overview" --download-attachments --download-html --output-dir ./pdfs
```

For discovery and repeatable future pulls:

```bash
uv run confluence-downloader list --space DOC --depth 2 --bulk-config pages.json
...
- Read `<output-dir>/downloaded_pages.md` to map page titles and IDs to PDF, HTML, and attachment filenames.
- Review the generated PDFs, not Confluence live pages, unless download failed and the user agrees to a fallback.
- With `--download-attachments`, attached files are under `<output-dir>/attachments/<page-slug>-<pageid>/`. A `_external-resources.md` there lists files embedded on the page but hosted outside Confluence (for example Google Docs or Slides); those cannot be downloaded with the Confluence token, so surface their URLs in the review instead of treating them as missing.
